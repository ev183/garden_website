...
> [!NOTE]
> This could certainly be done with a simpler (and cheaper!!) setup like S3+CloudFront I but wanted to dive into more advanced devops practices.

## Website design
Majority of the frontend code was produced via Claude Code. I update it weekly with information about my garden.
...
Everytime I push code to main (did not feel the need for branching as its just me working on this), I have a github action that...
 ...
 4. If not already done so, my ECS Service desired task count is set to 1. 

Instead of relying on GitHub-managed runners, I've implemented self-hosted runners using k3d clusters on my local machine. GitHub Actions Runner Controller (ARC), deployed via the official Helm chart, ensures a runner pod is always available for pipeline execution. I am considering building out a k3 cluster with 2 or 3 raspberry pi's and acheiving the same setup.
...
### ECS Task Definition
The task is designed to run on a service with a fargate launch type. The task definition is always set to use the latest image in my ECR repo (frequently updated as a part of my deploy pipeline). A task's specs are 0.25 vCPU and 512 mb of RAM.
